...
export default function StaffTasksPage() {
  ...
  const [drafts, setDrafts] = useState<
    ...
    >
  >({});
  const [completedByDayMain, setCompletedByDayMain] = useState<CompletedDay[]>(
    []
  );
  const [completedByDaySide, setCompletedByDaySide] = useState<CompletedDay[]>(
    []
  );
  ...
  const loadTasks = useCallback(async () => {
    ...
    const [mainRes, sideRes] = await Promise.all([
      fetch(
        `/api/staff/completed-daily?from=${today}&to=${today}&taskType=main`,
        { cache: "no-store" }
      ),
      fetch(
        `/api/staff/completed-daily?from=${today}&to=${today}&taskType=side`,
        { cache: "no-store" }
      ),
    ]);

    if (mainRes.status === 401 || sideRes.status === 401) {
      router.push("/staff/login");
      return;
    }

    if (!mainRes.ok) {
      setCompletedByDayMain([]);
    } else {
      const mainData = await mainRes.json();
      setCompletedByDayMain(
        Array.isArray(mainData?.days) ? mainData.days : []
      );
    }

    if (!sideRes.ok) {
      setCompletedByDaySide([]);
    } else {
      const sideData = await sideRes.json();
      setCompletedByDaySide(
        Array.isArray(sideData?.days) ? sideData.days : []
      );
    }
  }, [router]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button
          ...
          className={
            activeTab === "assigned" ? "crm-btn-primary" : "crm-btn-outline"
          }
        >
          Assigned Tasks
        </button>
        <button
          ...
          className={
            activeTab === "completed" ? "crm-btn-primary" : "crm-btn-outline"
          }
        >
          Completed By Day
        </button>
              ...
              return (
                <div key={task._id} className="crm-card">
                  ...
                  <div className="mt-6 grid gap-4 md:grid-cols-2">
                    ...
                    <label className="block">
                      <span className="crm-label crm-label-required">
                        Rate per tree
                      </span>
                      <input
                        type="number"
      ...
      {activeTab === "completed" && (
        ...
        <div>
          <p className="crm-pill">Completed</p>
          <h2 className="mt-3 text-2xl font-semibold text-[color:var(--ink)]">
            Completed Tasks By Day
          </h2>
          <p className="mt-1 text-sm text-[color:var(--muted)]">
            Showing customer name and trees for today only.
          </p>
        </div>

        <div className="space-y-6">
          <div>
            <p className="crm-pill">Main Tasks</p>
            {completedByDayMain.length === 0 ? (
              <div className="mt-3 crm-card">
                <p className="text-sm text-[color:var(--muted)]">
                  No completed main tasks today.
                </p>
              </div>
            ) : (
              <div className="mt-3 overflow-x-auto rounded-2xl border border-[color:var(--border)] bg-white/90">
                <table className="crm-table">
                  ...
                  <tbody className="divide-y divide-[color:var(--border)]">
                    {completedByDayMain.flatMap((day) =>
                      day.tasks.map((task, index) => (
                        <tr
                          key={`${day.date}-main-${index}`}
          ...
          <div>
            ...
            {completedByDaySide.length === 0 ? (
              <div className="mt-3 crm-card">
                <p className="text-sm text-[color:var(--muted)]">
                  No completed side tasks today.
                </p>
              </div>
            ) : (
              <div className="mt-3 overflow-x-auto rounded-2xl border border-[color:var(--border)] bg-white/90">
                <table className="crm-table">
                  ...
                  <tbody className="divide-y divide-[color:var(--border)]">
                    {completedByDaySide.flatMap((day) =>
                      day.tasks.map((task, index) => (
                        <tr
                          key={`${day.date}-side-${index}`}
                          className="hover:bg-white/70"
                        >
                          <td className="crm-td">
                            {new Date(day.date).toLocaleDateString()}
